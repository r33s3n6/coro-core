...
void freeproc(struct proc *p) {
    ...
    KERNEL_ASSERT(p->trapframe == nullptr, "p->trapfram is pointing somewhere, did you forget to free trapframe?");
    KERNEL_ASSERT(p->pagetable == nullptr, "p->pagetable is pointing somewhere, did you forget to free pagetable?");
    KERNEL_ASSERT(p->cwd == nullptr, "p->cwd is not nullptr, did you forget to release the inode?");
    KERNEL_ASSERT(p->waiting_target == nullptr, "p->cwd is waiting something");
    ...
    {
        KERNEL_ASSERT(p->files[i] == nullptr, "some file is not closed");
    }
...
struct file *get_proc_file_by_fd(struct proc *p, int fd) {
    if (p == nullptr) {
        panic("get_proc_file_by_fd: p is nullptr");
    }
