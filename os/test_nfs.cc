...
task<void> test_nfs2_coro(device_id_t device_id) {
    ...
    shared_ptr<dentry> file_dentry3_2 = *co_await kernel_dentry_cache.get_at(nullptr, "/test_dir/test_file3");
    ...
    shared_ptr<dentry> root = *co_await kernel_dentry_cache.get_at(file_dentry3_2, "../../");
    debugf("test_nfs_coro: root: '%s' %d", root->name.data(), root->get_inode()->inode_number);
    ...
    // read dir
    {
        ...
        while (auto dentry = *co_await dir_iterator) {
            debugf("test_nfs_coro: read dir: %s %d", dentry->name.data(), dentry->get_inode()->inode_number);
            count++;
        }
...
task<void> test_nfs_coro(device_id_t device_id) {
    ...
    {
        reference_guard<nfs::nfs_inode> root_inode_ref = *co_await root_inode->get_ref();
    ...
    // create simple file
    shared_ptr<dentry> file_dentry = *co_await kernel_dentry_cache.create(root_dentry, "test_file", nullptr);
...

    {
        ...
        int64 write_size = *co_await file_inode_ref->write("hello world", 0, 12);
        if (write_size != 12) {
    ...
    shared_ptr<dentry> file_dentry2 = *co_await kernel_dentry_cache.get_at(nullptr, "/test_file");
    ...
    // create dir
    shared_ptr<dentry> dir_dentry = *co_await kernel_dentry_cache.create(root_dentry, "test_dir", nullptr);
    ...
    // create 2 files
    shared_ptr<dentry> file_dentry3 = *co_await kernel_dentry_cache.create(dir_dentry, "test_file3", nullptr);
    shared_ptr<dentry> file_dentry4 = *co_await kernel_dentry_cache.create(dir_dentry, "test_file4", nullptr);
    ...
    // link file
    shared_ptr<dentry> file_dentry_link = *co_await kernel_dentry_cache.create(dir_dentry, "test_file3_link", nullptr);
...
void test_nfs2(void*){
    ...
    kernel_task_scheduler[0].schedule(test_nfs_coro(virtio_disk_id));
