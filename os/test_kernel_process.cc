...
void test_bind_core(void* arg) {
    ...
    for (uint64 i = 0; i < BIG_NUMBER; i++) {
        ...
        if (temp % (BIG_NUMBER) == 0) {
            ...
            debug_core("test_bind_core: pid:%d,expect core:%d core %d: %d",
                       task_id, __expect_core, __id, temp);
            if (__id != __expect_core) {
                debug_core(
                    "test_bind_core failed: pid: %d, expect core: %d, "
                    "real_core: %d",
                    task_id, __expect_core, __id);
                kernel_assert(false, "");
            }
        }
    }
    ...
    int task_id = c->get_kernel_process()->get_pid();
    uint64 __id = c->get_core_id();
    debug_core("test_bind_core: pid:%d,expect core:%d core %d", task_id,
               __expect_core, __id);
}
/*
task<void> test_disk_read(int block_no, int len) {
    debugf("test_disk_read: block_no: %d, len: %d", block_no, len);
    auto buffer_ref = co_await kernel_block_buffer.get_node(virtio_disk_id,
block_no); auto& buffer = *buffer_ref; std::optional<const uint8*> data_ptr =
co_await buffer.get_for_read(); if(!data_ptr) { co_return task_fail;
    }
    const uint8* data = *data_ptr;
    debug_core("test_disk_read: block_no: %d, len: %d, data: %s", block_no, len,
data);
...
task<void> test_disk_write(int block_no, uint8* buf, int len, int id) {
    ...
    auto buffer_ref = co_await kernel_block_buffer.get_node(virtio_disk_id,
block_no); auto& buffer = *buffer_ref;
    ...
    data[len] = '0' + id;
    old_bytes[len+1] = 0;
    debug_core("test_disk_write: block_no: %d, len: %d, old_bytes: %s",
block_no, len, old_bytes);
...
20+i); auto& buffer = *buffer_ref;
        ...
        data[0] = '0' + i;
        data[1] = 0;
        debug_core("test_buffer_reuse: block_no: %d, data: %s, &data: %p", 20+i,
data, (void*)data);
    }
    co_return task_ok;
}
...
task<void> test_coherence(device_id_t test_device_id) {
    ...
    if (test_device->capacity() < 1024) {
        debug_core("test_coherence: device %p: not enough space: %d",
                   test_device, test_device->capacity());
        co_return task_fail;
    }
    ...
    for (int i = 0; i < 1024; i++) {
        ...
        if (val != random_store[i]) {
            debug_core(
                "test_coherence: coherence 1 failed: i: %d, val: %p, "
                "random_store[i]: %p",
                i, (void*)val, (void*)random_store[i]);
            // kernel_assert(false, "");
            // break;
        }
    }
    ...
    for (int i = 0; i < 1024; i++) {
        ...
        if (val != random_store[i]) {
            debug_core(
                "test_coherence: coherence 2 failed: i: %d, val: %p, "
                "random_store[i]: %p",
                i, (void*)val, (void*)random_store[i]);
            // break;
            // kernel_assert(false, "");
...
class test_bdev_coherence {
    ...
    task<void> subtask_write(device_id_t test_device_id,
                             uint32 block_no,
                             uint64* random_store_ptr) {
        block_device* test_device = device::get<block_device>(test_device_id);
    ...
    task<void> subtask_read(device_id_t test_device_id,
                            uint32 block_no,
                            uint64 random_store,
                            int index,
                            int pass) {
        block_device* test_device = device::get<block_device>(test_device_id);
        {
            ...
            if (val != random_store) {
                debug_core(
                    "test_coherence 2: coherence %d failed: index: %d, val: "
                    "%p, expect: %p",
                    pass, index, (void*)val, (void*)random_store);
            }
        }
   ...
   public:
    ...
    task<void> get_test(device_id_t test_device_id) {
        ...
        for (int i = 0; i < 1024; i++) {
            kernel_task_queue.push(
                subtask_write(test_device_id, i, &random_store[i]));
        }
        ...
        for (int i = 0; i < 1024; i++) {
            kernel_task_queue.push(
                subtask_read(test_device_id, i, random_store[i], i, 1));
        }
        ...
        for (int i = 0; i < 1024; i++) {
            kernel_task_queue.push(
                subtask_read(test_device_id, i, random_store[i], i, 2));
        }
...
task<void> test_nfs_read(device_id_t device_id, uint32 times) {
    ...
    test_fs.print();
    {
        ...
        for (uint32 i = 0; i < times; i++) {
            char buf[48];
            const char* expect =
                "hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi\0hi"
                "\0";
            ...
            for (int j = 0; j < 48; j++) {
                if (buf[j] != expect[j]) {
                    debugf("test_make_nfs: buf[%d]: %d, expect[%d]: %d", j,
                           buf[j], j, expect[j]);
                    kernel_assert(false, "");
                }
            }
        ...
        file_dentry =
            *co_await kernel_dentry_cache.get_at(nullptr, "/test_file");
        shared_ptr<dentry> parent_dentry = file_dentry->parent;
        co_await (*co_await parent_dentry->get_inode()->get_ref())
            ->unlink(file_dentry);
        // kernel_dentry_cache.put(file_dentry);
        parent_dentry.reset();
    }
...
task<void> test_make_nfs(device_id_t device_id, uint32 times) {
...

    {
        ...
        for (uint32 i = 0; i < times; i++) {
            ...
            for (int j = 0; j < 48; j++) {
                if (buf[j] != expect[j]) {
                    debugf("test_make_nfs: i:%d buf[%d]: %d, expect[%d]: %d", i,
                           j, buf[j], j, expect[j]);
                    // kernel_assert(false, "");
                }
            }
...
task<void> test_make_nfs2(device_id_t device_id) {
    ...
    co_await test_fs.mount(device_id);
    test_fs.print();
    shared_ptr<inode> root = *co_await test_fs.get_root();
    ...
    kernel_assert(read_size == sizeof(nfs::dirent),
                  "read_size != sizeof(nfs::dirent)");

    debugf("test_make_nfs: first_file: %d: %s", first_file.inode_number,
           first_file.name);
    shared_ptr<nfs::nfs_inode> file_inode =
        *co_await test_fs.get_inode(first_file.inode_number);
    ...
    // test file rw
    simple_file _file((shared_ptr<inode>)file_inode);
    co_await _file.open();
    int64 file_size = *co_await _file.llseek(0, file::whence_t::END);
    ...
    for (uint32 i = 0; i < ekernel - skernel; i++) {
        if (buf[i] != skernel[i]) {
            debugf("test_make_nfs: i:%d buf[%d]: %d, skernel[%d]: %d", i, i,
                   buf[i], i, skernel[i]);
            // kernel_assert(false, "");
        }
    }
    ...
    file_inode->print();
    ((shared_ptr<nfs::nfs_inode>)root)->print();

    file_inode.reset(nullptr);

    // print superblock
    debugf("test_fs.sb.used_generic_blocks: %d",
           test_fs.sb.used_generic_blocks);
