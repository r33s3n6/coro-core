...
![trace_screenshot](assets/week13_1.png)
...
1. 简单写了一个读写磁盘的测例，创建102400个任务，都放入协程调度器，对1024个磁盘块进行读写。可以正常完成。
2. 由于协程贯穿了整个内核，目前还没有想到一个很优雅的办法去写多线程相关的测例，只能把测例用到的相关代码都用非协程风格改写，这个还没有完成。
3. 虽然同时把很多协程任务添加到了调度器中，并且不可能同时创建这么多线程并分配栈 (400M>128M)。但实际情况中这些任务没有必要一下全创建出来，创建出来的唯一好处是在可能的IO时可以进行计算从而提升整体性能，而如果要对比性能的好处，需要重新回到两者的耗时对比上。
4. 一次性创建大量的协程可能造成系统的死锁，因为虽然一个协程函数所需要的内存是可以提前得知的，但是他调用其他协程的所需空间是无法预分配的，当大量协程都在等待空闲空间时，此时只能让协程失败（kill）来解除死锁状态。
5. 协程的动态分配对内存分配器的要求较高，尝试引入了单线程版本（全局锁）的ptmalloc, 在空间占用方面有一定提升，但是消耗更多时间
